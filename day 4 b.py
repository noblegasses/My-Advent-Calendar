...
repeat = False 
for x in range (len(chopped)):
    if chopped[x][1][1] !='w' and chopped[x][1][1] !='s':
        for y in range(len(gaurdlist)):
            if chopped[x][1][1] == gaurdlist[y][0]:
                repeat = True
        if repeat == False:
            gaurdlist.append([chopped[x][1][1],[]])
    repeat= False
gaurdlist.sort()

for a in range (len (gaurdlist)):
    print (gaurdlist[a][0])
    target = gaurdlist[a][0]
    i=0
    gaurdcheck= 0
    timeschedule = []
    for x in range (len(chopped)):
        ...
        if chopped[x][1][1] == 'w' and tracking == True:
            time = int (chopped[x][1][0][1]) - int (sleep)
            for y in range (time):
              timeschedule[i-1][2].append(y+int(sleep))
        if chopped[x][1][1] != target and chopped[x][1][1] !='w' and chopped[x][1][1] !='s' and tracking == True:
            tracking = False
            gaurdcheck = chopped[x][1][1]
            i= i + 1
        ...
        for y in range(len(timeschedule)):
            for z in range (len (timeschedule[y][2])):
                if x== timeschedule[y][2][z]:
                    frequency[x][1] = int (frequency[x][1]) + 1
    frequency.sort(key = lambda x: x[1],reverse= True)
    gaurdlist[a][1]= frequency[0]
gaurdlist.sort(key = lambda x: x[1][1],reverse= True)
result = int(gaurdlist[0][0]) * int(gaurdlist[0][1][0])  
print (result)    